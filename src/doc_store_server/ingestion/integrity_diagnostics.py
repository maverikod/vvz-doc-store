...
class IntegrityRefused(Exception):
    ...
    The repository raises it only after the transaction that discarded the
    hierarchy *and* recorded the failed run and its span has committed, so a
    caller holding this exception can rely on ``diagnostic.span_id`` naming a row
    that exists. That is structural rather than a promise: the discard and the
    record are the same transaction, so there is no ordering in which one
    survives without the other. The ingestion boundary turns ``controlled_error``
    into what the caller receives, so the refusal has exactly one shape wherever
    it is reported.
    """
