...
def build_info_document(registry: CommandHelpRegistry) -> InfoDocument:
    ...
    sections = {
        ...
        "vectorization_runtime": (
            ...
            "semantic_chunk/sentence text to embed-client in text batches; document "
            "vectors are calculated as the arithmetic mean of active paragraph vectors "
            "for the document, falling back to chunk vectors only when no paragraph "
            "vectors are available; file vectors are calculated as the arithmetic mean "
            "of document vectors. This is the required method for large documents and "
            "files: doc-store does not send full document or file bodies to "
            "embed-client. It writes semantic_chunk_embeddings in database batches "
